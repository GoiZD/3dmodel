...
class WellnessARScreen extends StatefulWidget {
  ...
  final String arWellnessImage;  // New parameter

  const WellnessARScreen({
    Key? key,
    required this.productID,
    required this.arWellnessImage,  // Initialize the new parameter
  }) : super(key: key);
...
class _ARScreenState extends State<WellnessARScreen> {
  ...
  void initState() {
    ...
    _loadWellnessImage();  // Load the image from the passed parameter
  }
  ...
  void _loadWellnessImage() {
    setState(() {
      // Use the passed parameter directly
      _currentImagePath = widget.arWellnessImage;
    });
  }

  void _processCameraImage(CameraImage image) async {
    if (_isDetecting || !_controller.value.isInitialized) return;
    _isDetecting = true;
...
class PosePainter extends CustomPainter {
  ...
  final String imagePath;  // Correctly passing the image path
  ...
  Widget _createModelWidget(Offset point, Size modelSize) {
    ...
    return Positioned(
      ...
      top: y,
      child: Image.network(
        imagePath,  // Correctly using the passed imagePath
  ...
  Size _calculateModelSize(double shoulderDistance) {
    final double baseWidth = widgetSize.width * 0.3;
    final double baseHeight = baseWidth * 0.3;
    ...
    final double maxWidth = widgetSize.width * 0.3;
    final double minHeight = widgetSize.height * 0.1;
    final double maxHeight = widgetSize.height * 0.3;
