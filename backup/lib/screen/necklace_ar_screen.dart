...
class PosePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // This method can remain empty or you can add debug drawing if needed
  }
  ...
  List<Widget> get modelWidgets {
    ...
    for (var pose in poses) {
      ...
      if (leftShoulder != null && rightShoulder != null) {
        ...
        final faceWidth = (leftShoulder.x - rightShoulder.x).abs();
        final scaledFaceWidth = faceWidth * (widgetSize.width / imageSize.width);

        final modelSize = _calculateModelSize(scaledFaceWidth);
        final yAdjustment = _calculateYAdjustment(modelSize.width / widgetSize.width);

        widgets.add(_createModelWidget(scaledMidPoint, modelSize, yAdjustment));
      }
    }
    return widgets;
  }

  Widget _createModelWidget(Offset point, Size modelSize, double yAdjustment) {
    double x = point.dx - modelSize.width / 2;
    double y = point.dy - modelSize.height / 2 - yAdjustment;
  ...
  Size _calculateModelSize(double scaledFaceWidth) {
    final double baseScaleFactor = 0.22;
    final double minScaleFactor = 0.1;
    final double maxScaleFactor = 0.35;
    
    double scaleFactor = baseScaleFactor * (scaledFaceWidth / widgetSize.width);
  ...
  double _calculateYAdjustment(double scaleFactor) {
    final double baseScaleFactor = 0.22;
    final double minScaleFactor = 0.1;
    final double maxScaleFactor = 0.35;
    ...
    if (scaleFactor == baseScaleFactor) {
      ...
      return 30 + t * 50; // Interpolate between 30 and 80
    }
  }
