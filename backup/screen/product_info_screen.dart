...
import 'package:vcon_testing/helper/fetch_homeliving.dart';
...
import 'package:vcon_testing/components/product_bottom_sheet.dart';
...
class ProductInformation extends StatelessWidget {
  ...
  Future<Map<String, dynamic>?> _fetchProductData() async {
    try {
      ...
      final homelivingList = await fetchHomeLiving();
      // Combine both lists
      final combinedList = [
        ...watchList,
        ...wellnessList,
                    ...
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) => ModelViewerPage(
                          ...
                          textModelUrl: product['gitText'] ?? '', // Provide an empty string if gitText is null
  ...
  Future<void> _navigateToARScreen(BuildContext context, Map<String, dynamic> product) async {
  ...
  String arWellnessImage = product['arWellnessImage'];

  // Convert gs:// URL to https:// URL
  final ref = FirebaseStorage.instance.refFromURL(arWellnessImage);
  final httpsUrl = await ref.getDownloadURL();

  print('Converted arWellnessImage path: $httpsUrl');

  Navigator.push(
    context,
    MaterialPageRoute(
      builder: (context) => WellnessARScreen(
        productID: product['productID'],  // Assuming productID exists in the product map
        arWellnessImage: httpsUrl,         // Pass the converted HTTPS URL
      ),
    ),
  );
}
          ...
          if (productNumber != -1) {
            if (productNumber % 3 == 1) {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => EarringARScreen(
                    cameras: cameras,
                    productID: productID,
                  ),
                ),
